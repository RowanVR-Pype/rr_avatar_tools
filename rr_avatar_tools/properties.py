...
class AvatarExportGroupProperty(bpy.types.PropertyGroup):
...
class RoomieExportGroupProperty(bpy.types.PropertyGroup):
    uuid: bpy.props.StringProperty(default=str(uuid.uuid4()))
    ...
    def collection(self):
        collections = [
            c
            for c in rr_avatar_tools.data.collections
            if c.get("rec_room_roomie_uuid") == self.uuid
        ]
        return collections and collections[0] or None
...
def unregister():
    bpy.utils.unregister_class(AvatarExportGroupProperty)
    bpy.utils.unregister_class(RoomieExportGroupProperty)
