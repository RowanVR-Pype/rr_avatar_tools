...
class SCENE_PT_RRRoomieToolsDiagnosticsPanel(RecRoomRoomiePanel):
    ...
    def draw(self, context):
        ...
        if not context.scene.roomie_export_list:
            return

        # Only show diagnostics for selected avatar item
        prop = context.scene.roomie_export_list[context.scene.roomie_export_list_index]
        collections = [
            c
            for c in rr_avatar_tools.data.collections
            if c.get("rec_room_roomie_uuid") == prop.uuid
        ]
...
classes = (SCENE_PT_RRRoomieToolsDiagnosticsPanel,)
